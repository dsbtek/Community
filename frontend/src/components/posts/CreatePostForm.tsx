  ...
  const fetchUserGroups = async (): Promise<void> => {
    try {
      ...
      if (response.ok) {
        ...
        const userGroups = data.groups.filter((group: Group) => group.is_member);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        onSuccess?.(data.post);
        // Reset form
        setFormData({ title: '', content: '', group_id: selectedGroupId || 0 });
      } else {
  ...
  if (loadingGroups) {
    return (
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <p className="text-center mt-2 text-gray-600">Loading groups...</p>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        <div className="text-center py-8">
          <p className="text-gray-600 mb-4">
            You need to be a member of at least one group to create posts.
          </p>
          <button
            onClick={onCancel}
